...
userController.addHabit = async (req, res, next) => {
  ...
  const {name, reminder, frequency, isPrivate, videoName} = req.body;
  try {
    const query =
      'INSERT INTO habit (name, user_id, reminder, frequency, private, video_name) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *';
    const {
      rows: [habit],
    } = await db.query(query, [name, userId, reminder, frequency, isPrivate, videoName]);
...
userController.deleteHabit = async (req, res, next) => {
  ...
  try {
    const {rows} = await db.query('DELETE FROM habit WHERE id = $1 RETURNING *', [
      req.params.habitId,
    ]);
...
userController.completeHabit = async (req, res, next) => {
  if (!req.params.habitId) return next();
  try {
    await db.query('INSERT INTO habit_log (habit_id, date_completed) VALUES ($1, now())', [
      req.params.habitId,
    ]);
  } catch (e) {
    next(e.message);
...
userController.myTodayGoals = async (req, res, next) => {
  ...
  // }
  const curUserTodayQuery =
    'SELECT users_habits_join.* FROM users_habits_join JOIN user_habit_calendar ON user_habit_calendar.user_habits_join_id = users_habits_join._id WHERE user_habit_calendar.days_since_launch = $1 AND users_habits_join.users_id = $2';
  let todayGoals;
  try {
...
userController.acceptFriendRequest = async (req, res, next) => {
  ...
  const acceptRequestQry =
    'UPDATE friends SET request_accepted = TRUE WHERE _id = $1 AND friend_b = $2';
...
userController.removeFriend = async (req, res, next) => {
  ...
  const removeFriendQry =
    'DELETE FROM friends WHERE (_id = $1 AND friend_a = $2) OR (_id = $1 AND friend_b = $2)';
...
userController.findUser = async (req, res, next) => {
  ...
  const userQuery =
    'SELECT * from user_cache WHERE (lower(full_name) LIKE $1) OR (lower(email) = $1)';
